// script.js - full site script using provided image filenames
...
// fallback image (use an existing file from your repo)
const FALLBACK_IMG = 'logo.png'; // will display if product image fails to load

function safeImg(src) {
  const img = document.createElement('img');
  img.src = src;
  // set alt and loading lazy by default when used
  img.loading = 'lazy';
...
function renderSections() {
  ...
  sectionsData.forEach(sec => {
    ...
    // insert before the export section (as your HTML structure expects)
    const main = document.querySelector('main');
    const exportSection = document.getElementById('export');
    if (exportSection) main.insertBefore(section, exportSection);
...
function openModal(p) {
  ...
  if (modalImage) {
    modalImage.src = p.img || FALLBACK_IMG;
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // modal close behavior (click outside or close btn)
  ...
  // optional: keyboard Escape to close modal
